...
def dashboard(request):
    ...
    if request.user.is_authenticated:
        ...
        total_clientes=len(retorna_clientes_acc(request.user.id))
        clientes_alta = 0
        if total_clientes==0:
            indice=0
        else:
            clientes_alta=len(retorna_clientes_acc(request.user.id,'A'))
            indice=round(clientes_alta/total_clientes,2)

        return render(request, "dashboard.html",{'total_clientes':total_clientes,
                                                 'clientes_alta':clientes_alta,
